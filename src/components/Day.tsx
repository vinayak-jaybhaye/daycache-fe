...
export default function Day() {
  ...
  const [view, setView] = useState<'diary' | 'regular'>('diary');

  const entries = useDiaryStore(
    (state) => (date ? state.dayWithEntries[date]?.entries : undefined)
  )

  const fetchEntriesForDay = useDiaryStore(
    (state) => state.fetchEntriesForDay
  )
