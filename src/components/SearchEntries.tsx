            ...
            try {
                ...
                setSearchResults((prev) => {
                    ...
                    const uniqueEntries = Array.from(
                        new Map(newEntries.map((item) => [item.id, item])).values(),
                    );
            ...
            <div className=" overflow-hidden">
                <div className="space-y-2">
                    ...
                    <div className="relative group">
                        ...
                        <div className="absolute right-0 inset-y-0 pr-4 flex items-center">
                            <button
                    ...
                    {showFilters && (
                        <div
                            className={`bg-surface-raised p-2 grid grid-cols-1 sm:grid-cols-2 gap-2`}
                        >
                            <div className="space-y-1.5">
                                <label
                                    htmlFor="startDate"
                                    className="text-xs font-semibold uppercase tracking-wider text-text-muted ml-1"
                                >
                                    From
                                </label>
                                <div className="relative group">
                    ...
                    <div>
                        <div className="flex flex-col items-center justify-center py-12 text-center bg-bg-subtle/50 rounded-xl border border-dashed border-border-subtle">
                            ...
                            <p className="text-sm text-text-muted max-w-xs">
                                Try adjusting your search terms or date range to find what
                                you're looking for.
                            </p>
                        </div>
                        <DayList title="Explore Recent Activity" />
                    </div>
                )}

                {/* Results List */}
                <ul
                    className="space-y-3 overflow-y-auto max-h-[calc(100vh-150px)]"
                    ref={containerRef}
                >
                        ...
                        >
                            {entry.id === activeEntryId ? (
                                ...
                                >
                                    {/* Date Badge */}
                                    <div
                                        className="shrink-0 flex sm:flex-col items-center sm:items-start gap-2 sm:gap-0 sm:w-24"
                                        onClick={(e) => {
                                            e.stopPropagation();
