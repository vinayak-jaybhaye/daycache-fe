...
export default function DayList() {
  ...
  const hasMore = useDiaryStore((s: DiaryStore) => s.dayList.pagination.hasMore)
  ...
  return (
    <div className="max-w-4xl mx-auto py-6 px-4 space-y-4">
      <h1 className="font-heading text-2xl py-2 px-4 font-bold text-text-primary">Activity</h1>

      <div className="flex flex-col gap-2 h-[calc(100vh-100px)] overflow-y-auto" ref={containerRef}>
        {order.map((date) => {
          const day = items[date]
          if (!day) return null
          return <DayListItem key={date} day={day} active={date === selectedDate} />
        })}
        {hasMore && <div ref={loadMoreRef} className="flex items-center justify-center p-4 text-sm text-text-muted">
